...
public class TransactionalMessageServiceImpl implements TransactionalMessageService {
    ...
    private boolean needDiscard(MessageExt msgExt, int transactionCheckMax) {
        // 如果该消息回查的次数超过允许的最大回查次数，则该消息将被丢弃
    ...
    private boolean needSkip(MessageExt msgExt) {
        // 如果事务消息超过文件的过期时间，默认为 72小时，则跳过该消息
    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            // 获取 RMQ_SYS_TRANS_HALF_TOPIC 主题下的所有消息队列，然后依次处理
            ...
            for (MessageQueue messageQueue : msgQueues) {
                ...
                // 根据当前的处理进度依次从已处理队列拉取 32条消息，方便判断当前处理的消息是否已经处理过
                ...
                long i = halfOffset;
                while (true) {
                    // RocketMQ 处理任务的一个通用处理逻辑就是为每个任务一次只分配某个固定时长，超过该时长则需等待下次任务调度
                    ...
                    if (removeMap.containsKey(i)) {
                        ...
                        // 判断该消息是否需要丢弃或跳过
                        // needDiscard依据:如果该消息回查的次数超过允许的最大回查次数，则该消息将被丢弃
                        // needSkip 如果事务消息超过文件的过期时间，默认为 72小时，则跳过该消息
                        ...
                        long valueOfCurrentMinusBorn = System.currentTimeMillis() - msgExt.getBornTimestamp();
                        // checkImmunityTime：立即检测事务消息的时间，其设计的意义是，应用程序在发送事务消息后，事务不会马上提交，
                        // 该时间就是假设事务消息发送成功后，应用程序事务提交的时间，
                        // 在这段时间内，RocketMQ 任务事务未提交，故不应该在这个时间段向应用程序发送回查请求

                        // transactionTimeout：事务消息的超时时间，
                        // 这个时间是从OP拉取的消息的最后一条消息的存储时间与check方法开始的时间，
                        // 如果时间差超过了transactionTimeout，就算时间小于 checkImmunityTime 时间，也发送事务回查指令
                        long checkImmunityTime = transactionTimeout;
                        // 消息事务消息回查请求的最晚时间，单位为秒，指的是程序发送事务消息时，
                        // 可以指定该事务消息的有效时间，只有在这个时间内收到回查消息才有效，默认为 null
                        ...
                        // 判断是否需要发送事务回查消息
                        // 1)如果操作队列 (RMQ_SYS_TRANS_OP_HALF_TOPIC) 中没有已处理消息
                        // 并且已经超过应用程序事务结束时间即 transactionTimeOut值
                        ...
                        if (isNeedCheck) {
                            // 如果需要发送事务状态回查消息，
                            // 则先将消息再次发送到RMQ_SYS_TRANS_HALF_TOPIC主题中,发送成功则返回 true，否则返回 false
                            // >>>>>>>>>
