...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            while (true) {
                ...
                // 如果查找结果为 true 并且消息的长度等于 0，表示已到该文件的末尾
                    ...
                    // 如果还有下一个文件
                        ...
                        // 重置 processOffset、 mappedFileOffset继续循环
                ...
                // 如果查找结构为 false，表明该文件未填满所有消息，跳出循环
            ...
            // 更新 MappedFileQueue 的 flushedWhere 与 committedWhere 指针
    ...
    public void recoverAbnormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        // 如果未找到有效 MappedFile
        // 则设置 commitlog 目录的 flushedWhere、 committedWhere指针都为0，并销毁消息消费队列文件
        else {
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        // 首先判断文件的魔数，如果不是 MESSAGE_MAGIC_CODE，返回 false
        // 表示该文件不符合 commitlog 消息文件的存储格式
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        // 获取mappedFileQueue属性CopyOnWriteArrayList<MappedFile> mappedFiles的最后一个MappedFile
        // mappedFileQueue对应commitlog文件夹
        // mappedFile对应commitlog文件
        ...
        try {
            ...
            // 如果文件为空或者已经达到规定的大小，则创建一个新的文件，偏移量为0
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                // 构建 GroupCommitRequest
                GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes());
                // 将同步任务 GroupCommitRequest 提交到 GroupCommitService线程
                // >>>>>>>>>
                service.putRequest(request);
                // 调用阻塞等待刷盘结果，超时时间默认为 5s
                // >>>>>>>>>
    ...
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 如果距上次提交间隔超过 commitDataThoroughInterval
                if (begin >= (this.lastCommitTimestamp + commitDataThoroughInterval)) {
                    this.lastCommitTimestamp = begin;
                    // 本次提交忽略 commitCommitLogLeastPages 参数
                ...
                try {
                    ...
                    // 如果返回 false，并不是代表提交失败，而是只提交了一部分数据
                    if (!result) {
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 一次刷写任务至少包含页数， 如果待刷写数据不足，小于该参数配置的值，将忽略本次刷写任务，默认 4页
                ...
                // 如果距上次提交间隔超过 flushPhysicQueueThoroughInterval
                if (currentTimeMillis >= (this.lastFlushTimestamp + flushPhysicQueueThoroughInterval)) {
                    this.lastFlushTimestamp = currentTimeMillis;
                    // 本次刷盘任务、将忽略flushPhysicQueueLeastPages
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        // GroupCommitService 线程每次处理的 request容器，这是一个设计亮点，避免了任务提交与任务执行的锁冲突
        private volatile List<GroupCommitRequest> requestsRead = new ArrayList<GroupCommitRequest>();

        // 客户端提交同步刷盘任务到 GroupCommitService线程
        ...
        // GroupCommitService 提供读容器与写容器，这两个容器每执行完一次任务后，交互，继续消费任务
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                try {
                    // GroupCommitService每处理一批同步刷盘请求( requestsRead容器中请求)后“休息“ 10ms， 然后继续处理下一批
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 获取该消息的消息队列的偏移量，CommitLog.this.topicQueueTable 中保存了当前所有消息队列的当前待写入偏移量
            ...
            // 如果消息长度+END_FILE_MIN_BLANK_LENGTH大于 CommitLog文件的空闲空间，则返回异常
            ...
            // 将消息内容存储到 ByteBuffer 中
            byteBuffer.put(this.msgStoreItemMemory.array(), 0, msgLen);

            // 然后创建 AppendMessageResult 这里只是将消息存储在 MappedFile 对应的内存映射 Buffer中，并没有刷写到磁盘
